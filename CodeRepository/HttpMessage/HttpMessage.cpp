...
{
  attributes_.push_back(attrib);  //adds the message attributes
}
...
{
  Value val = "";
  size_t pos = findAttribute(name);    //trims the white spaces according to when it finds a colon
  if (pos < attributes_.size())        //gets the position
  {
    val = attributes_[pos].second;     //return stuff after the colon
  }
  return val;    //returns the second value after the colon
}
...
{
  return attributes_;   //returns the attributes
}
Attribute HttpMessage::parseAttribute(const std::string& src)
{
  Attribute attrib("", "");    //Pair<Name,Value>
  size_t pos = src.find(':');  //find ':' in the string passed as an argument
  if (pos == std::string::npos)  //npos means until the end of the string
    return attrib;
  std::string first = StringHelper::trim(src.substr(0, pos)); //from the 0th position till you get the length of 'pos'
...
void HttpMessage::addBody(const std::string& body)    //adds message contents
{
  body_.clear();
  for (auto ch : body)
    body_.push_back(ch);
...
void HttpMessage::addBody(size_t numBytes, byte* pBuffer)   //for files
{
