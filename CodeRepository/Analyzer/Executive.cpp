/////////////////////////////////////////////////////////////////////
// Executive.cpp - Performs type and dependency analysis and       //
//                 publishes the files received using the above   //
//                 performed analysis                             // 
// ver 1.0                                                        //
//-----------------------------------------------------------------//
//-----------------------------------------------------------------//
...
/*  Package Operations :
*  ===================
* Performs type and dependency analysis and       
* publishes the files received using the above   
*  performed analysis
*
*
...
*  ---------------------------
*  myExec() uses 
*    ProcessCommandLine(argc, argv);  to find .cpp and .h files from the command line arguments
*    showCommandLineArguments(argc, argv); to display the command line arguments
*    getSourceFiles();   to get the source files by traversing through the path
*	 processSourceCode(true);  to process the source code
*   calls objects of type table, dependency table and publisher to perform their respective functionalities.
*    
*
*  Maintanence History :
*--------------------
...
void CodeAnalysisExecutive::showActivity(const File& file)
{
  ...
  /*if (Rslt::running())
  {
	...
	//std::cout << std::left << "\r     Processing file:";
}
...
void CodeAnalysisExecutive::processSourceCode(bool showProc){
  for (auto file : cppHeaderFiles()){
    ...
    if (!configure_.Attach(file)){ std::ostringstream out;
  ...
  for (auto file : csharpFiles()){
    if (showProc)
      showActivity(file);
    pRepo_->package() = FileSystem::Path::getName(file);
    if (!configure_.Attach(file)){  std::ostringstream out; out << "\n  could not open file " << file << "\n";
...
void CodeAnalysisExecutive::req8()
{
	cout << "\n==================== \nREQUIREMENT 8 \n====================" << endl;
	cout << "Processing the command line" << endl;
	cout << "a) The path to the directory containing files to analyze : Testing *.h *.cpp Project2" << endl;
	cout << "b) There are four files : FileA.h, FileB.h, FileC.h and FileD.h" << endl;
	cout << "   FileB depends on FileA" << endl;
	cout << "   FileC depends on FileB" << endl;
	cout << "   FileA depends on FileC" <<endl;
	cout << "   FileD depends on FileC" << endl;
	cout << "c) Specification of the Xml results is stored in *Project2.xml* file" << endl;
}
int CodeAnalysisExecutive::myExec(int argc, char* argv[])
{
	...
	try {
		...
		p.createStyleSheet();  //creates the style sheet 
		p.createJavaScript();
		p.publishAllFiles();
	}
	catch (std::exception& except)
	{
		exec.flushLogger(); std::cout << "\n\n  caught exception in Executive::main: " + std::string(except.what()) + "\n\n";
		exec.stopLogger();
		return 1;
	}return 0;
}
...
{using namespace CodeAnalysis;
  CodeAnalysisExecutive exec;try {bool succeeded = exec.ProcessCommandLine(argc, argv);
    if (!succeeded) { return 1;}
