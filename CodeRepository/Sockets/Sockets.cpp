...
*/
bool Socket::send(size_t bytes, byte* buffer)    //sends bytes
{
  size_t bytesSent = 0, bytesLeft = bytes;
  byte* pBuf = buffer;   // pBuf contains the message byte by byte
  while (bytesLeft > 0)
  {
    bytesSent = ::send(socket_, pBuf, bytesLeft, 0);     //WinSock2.h -->giving a count of how many bytes are sent 
    if (socket_ == INVALID_SOCKET || bytesSent == 0)
      return false;
    bytesLeft -= bytesSent;  //bytesLeft 
    pBuf += bytesSent;   //append whatever hasn't been sent to pBuf
  }
  return true;
}
//----< recv buffer >--------------------------------------------------------
...
bool SocketListener::listen()
{
  ...
  Show::write("\n  TCP listening socket setup complete");
  return true;
}
